...
function Lectures() {
  ...
  const listFn = useServerFn(listLectureFiles);
  const listSubjectsFn = useServerFn(listSubjects);
  const createFn = useServerFn(createLectureFile);
  const deleteFn = useServerFn(deleteLectureFile);
  const signFn = useServerFn(signLectureFileUrl);

  const { data: rows = [], isLoading } = useQuery({
    queryKey: ["lecture_files"],
    queryFn: () => listFn(),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["lecture_files"] });
    },
  ...
  async function handleUpload() {
    ...
    if (!ALLOWED.includes(ext as any)) {
      toast.error("Only PDF, PPT, PPTX, DOC, DOCX are allowed.");
      return;
    }
    setUploading(true);
    try {
      const path = `${selectedSubject}/${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, "_")}`;
      ...
      >
        <DialogContent className="sm:max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpload} disabled={uploading || !file || !selectedSubject}>
